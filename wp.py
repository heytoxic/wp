...
def wa_send(client, jid, msg: dict) -> bool:
    """Text aur Media dono send karne ke liye — Fixed for Video & APK."""
    mtype = msg.get("type", "text")
    text = msg.get("text", "")
    file_path = msg.get("file_path", "")

    # --- Text aur Link Sending ---
    if mtype == "text":
        try:
    ...
    # --- Media Sending (Photo, Video, Document/APK) ---
    if mtype in ["photo", "video", "document"]:
        import os, mimetypes
        if not os.path.exists(file_path):
            print(f"[wa_send] Media file missing: {file_path}")
            return False
        ...
        try:
            ...
            # --- APK aur Document ke liye MIME FIX (BIN Issue Solve) ---
            ...
            # --- Build Media Messages ---
            if mtype == "photo":
                media_msg = client.build_image_message(media_data, caption=text, mime_type=mime)
            
            elif mtype == "video":
                # Video ke liye build_video_message use hota hai
                # Agar caption hai toh wo yahan caption=text se set ho jayega
            ...
            else: # APK ya Document
                media_msg = client.build_document_message(
                    media_data, 
                    title=filename, 
                    mimetype=mime
                )
            ...
            print(f"[wa_send] Media Error: {e}")
            # Fallback: Agar media build fail ho toh text bhej do
            try:
                if text: client.send_message(jid, text)
            except: pass
            return False
